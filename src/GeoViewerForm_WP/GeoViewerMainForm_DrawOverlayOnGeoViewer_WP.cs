...
public partial class GeoViewerMainForm : Form
{
	...
	//	viewer.AddOverlay("test_map_ol", img_map_data, 1000.0, 0.5f);
		...
		//	var ol_e_lg = new CLg(ToDecimalDeg(130, 25, 0.0)); var ol_e_lt = new CLt(ToDecimalDeg(33, 43, 0.0));
			...
			var p_from = ol.ToPointOnOverlay(ToWP(MeshZoomLevel, new CLgLt(ol_s_lg, ol_s_lt)));
			var p_to   = ol.ToPointOnOverlay(ToWP(MeshZoomLevel, new CLgLt(ol_e_lg, ol_e_lt)));
...
StopWatch.Lap("before IsObserve in C++");
			var ol_1 = COverlay_WP.MakeVisibilityOverlay(op_lglt, new CLgLt(obj_s_lg, obj_s_lt), new CLgLt(obj_e_lg, obj_e_lt), MeshZoomLevel, 10.0);
StopWatch.Lap("after  IsObserve in C++");

StopWatch.Lap("before IsObserveMP in C++");
			var ol_2 = COverlay_WP.MakeVisibilityOverlayMP(op_lglt, new CLgLt(obj_s_lg, obj_s_lt), new CLgLt(obj_e_lg, obj_e_lt), MeshZoomLevel, 10.0);
...
/*
			...
			for(var obj_y_wp = 0; obj_y_wp <= ol_h_wpr; obj_y_wp += dot_size_wpr)
			{
				...
				//	if(!IsObserve(MeshZoomLevel, op_lglt, obj_lglt)) // C# 版
					if(!IsObserve(op_lglt, obj_lglt, MeshZoomLevel)) // C++版
