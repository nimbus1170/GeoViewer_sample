...
namespace GeoViewer_sample
{
	public partial class GeoViewerMainForm : Form
	{
		...
		readonly Dictionary<string, CStyle> Styles = [];
		...
		void ReadDrawKMLFromFile(in string kml_fname)
		{
			...
			var local_cfg_fname = Path.GetDirectoryName(kml_fname) + "\\default.cfg.xml";
	...
	//				var name = name0 + "_" + placemark.Attributes["id"]?.Value;
			...
			ShowLog($"{name}\r\n");
			ShowLog($"  ポイント数 : {points_n  }\r\n");
			ShowLog($"    ライン数 : {lines_n   }\r\n");
			ShowLog($"  ポリゴン数 : {polygons_n}\r\n");
		}
