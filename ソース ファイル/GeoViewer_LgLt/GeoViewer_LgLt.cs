...
*/		}
	...
	//	if(!(File.Exists(gsi_geoid_model_file))) throw new Exception("geoid model file not found");
		...
		GeoViewer_LgLt = new CGeoViewer_LgLt(viewer_params, scene_cfg, controller_parts, Info, Profiler);
		...
		viewer_form	   .Viewer = GeoViewer_LgLt;
		controller_form.Viewer = GeoViewer_LgLt;
		...
		var cfg_form = new GeoViewerCfgForm(GeoViewer_LgLt);
		...
		if(false)
		{
			...
			GeoViewer_LgLt.AddOverlayPlane(img_map_data, 1000.0, 0.5f);
		...
		{ 
			...
			GeoViewer_LgLt.AddOverlayPlane
